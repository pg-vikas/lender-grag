  ...
  return (
    <AppShell>
      <div className="space-y-5">
        <div>
          <h1 className="text-[28px] font-black text-white tracking-tight">Borrowers</h1>
          <p className="text-[15px] font-medium text-white/45 mt-1">Active borrower records</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 max-w-[400px] flex items-center gap-2 h-10 px-3 rounded-lg bg-white/[0.04] border border-white/[0.06]">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search borrowers..." className="flex-1 bg-transparent text-white text-[14px] font-medium placeholder:text-white/25 focus:outline-none" data-testid="input-borrowers-search" />
          </div>
              ...
              <tbody>
                {filtered.map((b) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3 text-[14px] font-semibold text-white/90">{b.fullName}</td>
                      ...
                      <td className="px-4 py-3 text-[13px] font-semibold text-white/65">{lf ? `$${(lf.amount / 1000).toFixed(0)}K` : "—"}</td>
                      <td className="px-4 py-3 text-[13px] font-medium text-white/60">{getLOName(b.assignedLOId)}</td>
                      <td className="px-4 py-3 text-[13px] font-medium text-white/55">{getProcessorName(b.processorId)}</td>
                      <td className="px-4 py-3 text-[13px] font-medium text-white/55">{getBranchName(b.branchId)}</td>
                      <td className="px-4 py-3 text-[12px] font-medium text-white/40">{new Date(b.lastActivityAt).toLocaleDateString()}</td>
