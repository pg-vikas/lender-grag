  ...
  return (
    <AppShell>
      <div className="space-y-5">
        <div>
          <h1 className="text-[28px] font-black text-white tracking-tight">Leads</h1>
          <p className="text-[15px] font-medium text-white/45 mt-1">Lead management center</p>
        </div>
          ...
          {[
            ...
            <div key={s.label} className="bg-[#111] border border-white/[0.06] rounded-xl p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] font-semibold text-white/35">{s.label}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex-1 min-w-[200px] flex items-center gap-2 h-10 px-3 rounded-lg bg-white/[0.04] border border-white/[0.06]">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search leads..." className="flex-1 bg-transparent text-white text-[14px] font-medium placeholder:text-white/25 focus:outline-none" data-testid="input-leads-search" />
          </div>
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="h-10 px-3 rounded-lg bg-white/[0.04] border border-white/[0.06] text-white/60 text-[13px] font-medium focus:outline-none"
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span className="text-[14px] font-semibold text-white/90">{l.firstName} {l.lastName}</span>
                    ...
                    <td className="px-4 py-3 text-[13px] font-medium text-white/55">{l.source}</td>
                    <td className="px-4 py-3"><StatusBadge status={l.status} /></td>
                    <td className="px-4 py-3 text-[13px] font-medium text-white/60">{getLOName(l.assignedLOId)}</td>
                    <td className="px-4 py-3 text-[13px] font-medium text-white/55">{l.phone}</td>
                    <td className="px-4 py-3 text-[13px] font-medium text-white/55">{l.email}</td>
                    <td className="px-4 py-3 text-[12px] font-medium text-white/40">{new Date(l.createdAt).toLocaleDateString()}</td>
                    <td className="px-4 py-3 text-[12px] font-medium text-white/40">{l.lastContactAt ? new Date(l.lastContactAt).toLocaleDateString() : "—"}</td>
                    <td className="px-4 py-3 text-[13px] font-semibold text-white/65">${(l.estimatedLoanAmount / 1000).toFixed(0)}K</td>
                    <td className="px-4 py-3">
